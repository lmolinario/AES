...
(Core 0 / Core 1 cooperation) and data-level parallelism (SIMD) through
compiler auto-vectorization using ARM NEON instructions applied to the
serial baseline.
...
├── Lab 4 Report.pdf                 → Experimental result Report
├── lab4_results.txt                 → brief text file Experimental result 
...
Execution time is measured using the **ARM Cortex-A9 Global Timer**, a
free-running hardware counter shared between both cores of the Zynq-7000
processing system.

In this implementation, only the **lower 32 bits** of the Global Timer
counter are read.  
This choice is sufficient for the short execution window of the experiment
and avoids the need to manage 64-bit overflows.

The timer is accessed through the Xilinx BSP base address
`XPAR_GLOBAL_TMR_BASEADDR`, using an explicit offset of `0x00`, corresponding
to the lower counter register.
...
```
Timing measurements are performed by sampling the timer immediately before
and after the code section under test:
...
```
The difference t_end - t_start represents the number of clock cycles
elapsed during execution.

The Global Timer frequency on the Zybo Z7 platform is approximately:

```
Global Timer ≈ 333 MHz
```
All reported execution times are therefore expressed in clock cycles.
Conversion to absolute time units (e.g., microseconds) is intentionally
avoided, as the objective of the experiment is a relative performance
comparison between serial and parallel implementations.

---
 **Notes on Timing Accuracy**

* Instruction and data caches are disabled to ensure deterministic timing
  and immediate visibility of shared-memory updates.
* UART output and synchronization via GPIO are kept **outside** the measured
  timing windows.
* Both serial and parallel executions are measured using the **same timer
  source**, ensuring a fair comparison.
...
<p align="center"><i>UART output showing correct parallel execution on both cores (Master and Worker from Example Code). </i></p>


---
<p align="center">
  <img src="../img/lab4/lab4_10.png" alt="Conceptual functionality of convolution" width="650"/>
</p>
<p align="center"><i>Output produced by both cores (Master + Worker): ARRAY_SIZE = 10</i></p>

---
<p align="center">
  <img src="../img/lab4/lab4_100.png" alt="Conceptual functionality of convolution" width="650"/>
</p>
<p align="center"><i>Output produced by both cores (Master + Worker): ARRAY_SIZE = 100
</i></p>
...
<p align="center"><i>Output produced by both cores (Master + Worker): ARRAY_SIZE = 1000</i></p>
...
<p align="center"><i>Output produced by both cores (Master + Worker): ARRAY_SIZE = 10000</i></p>
...
(`ARRAY_SIZE = 10`, `100`, `1000`, `10000`) using the ARM Cortex-A9 Global Timer.
...
## Lab 4 – Experimental Results (Summary)

The experimental evaluation of Lab 4 compares serial and parallel execution of a
vector addition kernel on a dual-core ARM Cortex-A9 platform, as well as the impact
of SIMD auto-vectorization using ARM NEON instructions.

Execution time was measured using the ARM Global Timer for different array sizes:
ARRAY_SIZE = 10, 100, 1000, and 10000.

The parallel implementation, where Core 0 and Core 1 process half of the vector
each, does not provide a significant execution-time improvement compared to the
serial baseline. In some cases, parallel execution exhibits similar or slightly
higher execution times.

This behavior is explained by the memory-bound nature of the workload: both cores
access the same shared DDR memory, leading to bandwidth contention that dominates
execution time. Synchronization overhead is minimal and does not represent the main
limiting factor.

The serial baseline was successfully auto-vectorized by the compiler using ARM NEON
SIMD instructions, as confirmed by assembly inspection. While SIMD reduces the number
of executed instructions, it does not overcome the memory bandwidth limitation of
the system.
