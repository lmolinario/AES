...
weights and biases, improving numerical stability and precision around zero, while preserving
sign symmetry and compatibility with 16-bit arithmetic  while preserving
...
By allocating one additional bit to the integer part, this format increases the
dynamic range available during accumulation, reducing the likelihood of
saturation effects while preserving compatibility with 16-bit integer arithmetic.
...
The Tip~1+Tip~2 configuration further reduces memory usage by adopting Q1.7
weights and biases.
However, as parameter transmission occurs only once at startup, this
optimization does not significantly affect runtime latency.
The observed performance gains in this configuration are therefore almost
entirely attributable to the reduced UART input bandwidth introduced by Tip~1.
...
A limited number of misclassifications were observed during runtime execution
with UART-streamed inputs, indicating a trade-off between reduced precision and
...
The optimized configuration therefore yields a more balanced and robust system
design, while fully satisfying real-time requirements and preserving deterministic
execution behavior.
...
To summarize the key findings of the performance evaluation and their implications
from a system design perspective, Table~\ref{tab:design_implications} reports a
compact mapping between experimental observations and the corresponding design
...
\hline
Reduced-precision parameters preserve correctness &
Q1.7 weights and biases do not affect inference time or classification behavior &
Alternative fixed-point formats can improve numerical robustness without compromising timing predictability \\
\hline
...
floating-point operations, ensuring deterministic execution behavior and predictable
...
and alternative fixed-point formats for network parameters, further improved
communication efficiency and numerical robustness without modifying the core
inference algorithm.
...
The complete \texttt{results.txt} file contains repeated iterations for all test
