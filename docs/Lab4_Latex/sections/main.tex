...
(\texttt{ARRAY\_SIZE = 100}, \texttt{1000}, and \texttt{10000}) to evaluate the
scalability of the proposed approach.

The experimental results show that parallel execution does not yield a significant
speedup compared to the serial baseline. In some cases, the parallel implementation
exhibits execution times comparable to or slightly higher than the serial version.
This behavior is consistent across all tested array sizes.

From an architectural perspective, these results are expected and consistent with
the state of the art. The vector addition kernel is strongly memory-bound, and both
cores concurrently access the same shared DDR memory. As a consequence, memory
bandwidth contention dominates execution time, limiting the benefits of task-level
parallelism despite the even workload partitioning and minimal synchronization
overhead.
\begin{figure}[H]
    \centering
...
\texttt{ARRAY\_SIZE = 100}, \texttt{1000}, and \texttt{10000}.
