 ...
 * -----------------------
 * src      = buffer input (raster RGB)
 * dst      = buffer output (raster RGB)
 * count    = N = width * height * 3
 * maxval   = header maxval (≤255)
 *
 * Applica equalizzazione indipendente per i livelli 0..maxval.
 */
static void histogram_equalization(const uint8_t *src,
                                   uint8_t *dst,
                                   uint32_t count,
                                   uint32_t maxval)
{
    ...
    if (maxval == 0) {
        /* immagine non valida → copia */
        for (i = 0; i < count; i++)
            dst[i] = src[i];
        return;
    }

    /* Total pixels = count (RGB interleaved) */
    uint32_t total = count;  // count = width * height * 3

    /* -----------------------------
     * STEP 1–2: Calcolo istogramma
     * ----------------------------- */
    for (i = 0; i < count; i++) {
        uint8_t p = src[i];
        hist[p]++;
    ...
    /* ---------------------------------------
     * STEP 4: Trova primo valore CDF non-zero
     * --------------------------------------- */
    uint32_t cdf_min = 0;
    for (i = 0; i <= maxval; i++) {
        if (cdf[i] != 0) {
    ...
    /* ---------------------------------------
     * STEP 4: Crea mappatura normalizzata
     *
     * map[i] = round( (cdf[i] - cdf_min) / (total - cdf_min) * maxval )
     * --------------------------------------- */
    uint32_t denom = total - cdf_min;
    if (denom == 0) {
        /* immagine uniforme → mappa identità */
        for (i = 0; i <= maxval; i++)
            map[i] = (uint8_t)i;
    } else {
        for (i = 0; i <= maxval; i++) {
            if (cdf[i] < cdf_min) {
                ...
                float norm = (float)(cdf[i] - cdf_min) / (float)denom;
                int value = (int)(norm * maxval + 0.5f);
                if (value < 0) value = 0;
                if (value > (int)maxval) value = maxval;
                map[i] = (uint8_t)value;
            }
        }
    }
    ...
    /* ---------------------------------------
     * STEP 5: Applica mappa a ogni pixel
     * --------------------------------------- */
    for (i = 0; i < count; i++) {
        uint8_t p = src[i];
        dst[i] = map[p];
...
static void ppm_microserver_once(void) {
    ...
    histogram_equalization(img_in, img_out, num_bytes, maxval);
