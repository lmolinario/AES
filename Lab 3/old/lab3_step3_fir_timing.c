...
/* FIR COEFFICIENTS (20 tap LP for test) */
#define N_TAP_BASE  20
float LP[N_TAP_BASE] = {
...
int AudioInitialize()
{
    ...
    TimerInitialize(XPAR_SCUTIMER_DEVICE_ID);
...
/* ------------------- FIR FILTER (software) ------------------- */
float fir(float x, float *h, float *buf, int N)
{
    for (int i = N - 1; i > 0; i--)
        buf[i] = buf[i - 1];
...
{
    init_platform();
    xil_printf("\n===== LAB 3 – STEP 3: Auto Tap Detection =====\n\r");

    AudioInitialize();

    volatile int *switches = (int*)SWI_BASE_ADDR;

    float bufL[200] = {0};   // spazio per test fino a 200 tap
    float bufR[200] = {0};

    const float Fs = 48000.0f;          // approx real Zybo sampling
    const float CPU_freq = 333000000.0; // global timer clock (333 MHz)
    float budget_cycles = CPU_freq / Fs;

    xil_printf("Sampling Fs ≈ %.1f Hz\n", Fs);
    xil_printf("Budget per sample = %.1f cycles\n\n", budget_cycles);

    int N = N_TAP_BASE; // numero tap attivi

    while (1)
    {
        /* Cache OFF if SW1 is ON */
        if (*switches & 0x2)
        {
            Xil_DCacheDisable();
            Xil_ICacheDisable();
        ...
        if (*switches & 0x1)
        {
            /* --------- Measure TAP execution time --------- */
            u32 t0 = Xil_In32(GLOBAL_TMR_BASEADDR + 0x4);
            float yL = fir(xL, LP, bufL, N);
            float yR = fir(xR, LP, bufR, N);
            u32 t1 = Xil_In32(GLOBAL_TMR_BASEADDR + 0x4);

            u32 cycles = t1 - t0;
            float cycles_per_tap = (float)cycles / N;
            float max_taps = budget_cycles / cycles_per_tap;

            xil_printf("N=%d | FIR=%u cycles | %.2f cyc/tap | MAX usable taps=%d\n",
                       N, cycles, cycles_per_tap, (int)max_taps);
